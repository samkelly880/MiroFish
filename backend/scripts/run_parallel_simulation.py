...
Runs Twitter and Reddit simulations together from the same config file

Features:
- Dual-platform (Twitter + Reddit) parallel simulation
- After simulation finishes, keep environments alive and wait for commands
- Receive Interview commands via IPC
...
# Works around OASIS third-party libs opening files without an encoding
# ============================================================
import sys
import os
...
if sys.platform == 'win32':
    ...
    # Affects all open() calls that omit encoding
    os.environ.setdefault('PYTHONUTF8', '1')
    os.environ.setdefault('PYTHONIOENCODING', 'utf-8')
    
    # Reconfigure stdout/stderr as UTF-8 (avoid console mojibake)
    if hasattr(sys.stdout, 'reconfigure'):
        sys.stdout.reconfigure(encoding='utf-8', errors='replace')
    if hasattr(sys.stderr, 'reconfigure'):
        sys.stderr.reconfigure(encoding='utf-8', errors='replace')
    
    # Force default encoding for open()
    # Note: ideally set at Python startup; runtime changes may not stick
    # so we also monkey-patch the builtin open function
    import builtins
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None, 
        ...
        """
        Wrap open() so text mode defaults to UTF-8 encoding
        Fixes third-party libs (e.g. OASIS) that omit encoding when reading files
        """
        # Only set a default encoding for text mode without an explicit encoding
...
# This script lives under backend/scripts/
...
# Load project-root .env (includes LLM_API_KEY and related settings)
...
class MaxTokensWarningFilter(logging.Filter):
    """Filter camel-ai max_tokens warnings (we intentionally omit max_tokens so the model decides)"""
    
    def filter(self, record):
        # Drop log records that warn about max_tokens
        if "max_tokens" in record.getMessage() and "Invalid or missing" in record.getMessage():
            return False
        return True


# Install filter at import time so it applies before camel runs
...
def disable_oasis_logging():
    ...
    OASIS logs every agent observation/action; we use our own action_logger instead
    """
    ...
    for logger_name in oasis_loggers:
        ...
        logger.setLevel(logging.CRITICAL)  # Only keep critical errors
...
# Twitter available actions (INTERVIEW excluded; only via ManualAction)
TWITTER_ACTIONS = [
...
# Reddit available actions (INTERVIEW excluded; only via ManualAction)
REDDIT_ACTIONS = [
    ActionType.LIKE_POST,
...
class ParallelIPCHandler:
    ...
    Manages both platform environments and Interview commands
    """
    ...
    def poll_command(self) -> Optional[Dict[str, Any]]:
        ...
        # Collect command files (sorted by mtime)
        command_files = []
    ...
    def _get_env_and_graph(self, platform: str):
        """
        Get env and agent_graph for a platform
    ...
    async def _interview_single_platform(self, agent_id: int, prompt: str, platform: str) -> Dict[str, Any]:
        """
        Run Interview on a single platform
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Optional platform filter
                ...
                - None/omitted: Interview both platforms and return combined results
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Default platform (overridable per interview item)
                - "twitter": Interview Twitter only
                - "reddit": Interview Reddit only
                - None/omitted: Interview both platforms for each agent
        """
        # Group by platform
        twitter_interviews = []
        reddit_interviews = []
        both_platforms_interviews = []  # Interviews that should hit both platforms
        ...
        # Split both_platforms_interviews onto both platforms
    ...
    async def process_commands(self) -> bool:
        ...
        if command_type == CommandType.INTERVIEW:
            ...
            self.send_response(command_id, "completed", result={"message": "Environment will shut down"})
            return False
...
# Action type map (DB name -> canonical name)
ACTION_TYPE_MAP = {
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    """
    Build agent_id -> entity_name map from simulation_config
    
    Lets actions.jsonl show real entity names instead of codes like "Agent_0"
    
    Args:
        config: Contents of simulation_config.json
        
    Returns:
        agent_id -> entity_name mapping dict
    """
    agent_names = {}
...
def fetch_new_actions_from_db(
    ...
    """
    Fetch new action records from the DB and enrich with full context
    
    Args:
        db_path: Database file path
        last_rowid: Last seen max rowid (prefer rowid over created_at; platforms format created_at differently)
        agent_names: agent_id -> agent_name map
        
    Returns:
        (actions_list, new_last_rowid)
        - actions_list: Action list; each item has agent_id, agent_name, action_type, action_args (with context)
    ...
    try:
        ...
        # Track processed rows via rowid (SQLite built-in autoincrement)
        # Avoids created_at format differences (Twitter int vs Reddit datetime string)
        cursor.execute("""
        ...
        for rowid, user_id, action, info_json in cursor.fetchall():
            ...
            # Slim action_args to key fields (keep full content, no truncation)
            simplified_args = {}
            ...
            # Enrich with context (post content, usernames, etc.)
...
def _enrich_action_context(
    ...
    """
    Enrich an action with context (post content, usernames, etc.)
    ...
    try:
        ...
        # Repost: add original post content and author
            ...
            if new_post_id:
                # For reposts, original_post_id points at the original
                cursor.execute("""
        ...
        # Quote post: add original content, author, and quote text
            ...
            # Fetch quote comment content (quote_content)
            if new_post_id:
                cursor.execute("""
...
def get_active_agents_for_round(
    ...
    """Decide which Agents to activate this round from time and config"""
...
async def run_twitter_simulation(
    ...
    Args:
        ...
        max_rounds: Max simulation rounds (optional; truncates long runs)
    ...
    # Twitter uses the primary LLM config
    ...
    # Map agent IDs to real entity names from config (not default Agent_X)
    agent_names = get_agent_names_from_config(config)
    # Fall back to OASIS default names when config has no entry
    ...
    total_actions = 0
    last_rowid = 0  # Track last processed DB row (rowid avoids created_at format differences)
    ...
    for round_num in range(total_rounds):
        ...
        if not active_agents:
            # Still log round end when no agents are active (actions_count=0)
            if action_logger:
        ...
        # Read and log actions that actually ran from the DB
    ...
    # Keep environment open for Interview use
...
async def run_reddit_simulation(
    ...
    # Reddit uses boost LLM config when available, otherwise primary
...
async def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help='Max simulation rounds (optional; truncates long runs)'
    )
    parser.add_argument(
        '--no-wait',
        action='store_true',
        default=False,
        help='Close environment immediately after simulation; do not wait for commands'
    )
    
    args = parser.parse_args()
    
    # Create shutdown event at the start of main so the whole process can exit cleanly
    ...
    # Initialize logging (disable OASIS logs; clear old files)
    ...
    if args.max_rounds:
        ...
        if args.max_rounds < config_total_rounds:
            log_manager.info(f"  - Rounds to execute: {args.max_rounds} (truncated)")
    ...
    # Hold results for both platforms
    ...
    if args.twitter_only:
        ...
        # Run in parallel (each platform has its own logger)
    ...
    # Optionally enter wait-for-commands mode
    if wait_for_commands:
        log_manager.info("")
        log_manager.info("=" * 60)
        log_manager.info("Entering wait-for-commands mode - environment stays alive")
        ...
        try:
            while not _shutdown_event.is_set():
                ...
                if not should_continue:
                    break
                # Prefer wait_for over sleep so shutdown_event is honored
                try:
...
def setup_signal_handlers(loop=None):
    """
    Install signal handlers so SIGTERM/SIGINT exit cleanly
    
    Persistent simulation: after the loop finishes, stay up for interview commands.
    On termination:
    1. Signal the asyncio loop to stop waiting
    2. Allow normal resource cleanup (DB, environments, etc.)
    3. Then exit
    """
    def signal_handler(signum, frame):
        ...
        if not _cleanup_done:
            _cleanup_done = True
            # Set event so the asyncio loop can exit and clean up
            if _shutdown_event:
                _shutdown_event.set()
        
        # Avoid sys.exit() on first signal; let asyncio exit and clean up
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Clean up multiprocessing resource tracker (avoid exit warnings)
        try:
