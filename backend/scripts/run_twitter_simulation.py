...
Reads parameters from a config file and runs the simulation end-to-end

Features:
- After simulation finishes, keep the environment alive and wait for commands
...
class IPCHandler:
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict]) -> bool:
        ...
        try:
            # Build actions dict
            actions = {}
            agent_prompts = {}  # Track prompt per agent
...
class TwitterSimulationRunner:
    ...
    def _get_profile_path(self) -> str:
        """Get Profile file path (OASIS Twitter uses CSV)"""
    ...
    def _create_model(self):
        ...
        Primary: grok-cli via OasisLLMBridge (JSON action tool-calls).
        Secondary: openai-compatible HTTP when LLM_PROVIDER=openai-compatible.
        """
        from app.providers.factory import get_provider_name
        from app.providers.oasis_bridge import create_oasis_model

        provider = get_provider_name()
        print(f"LLM config: provider={provider}")
        return create_oasis_model(self.config)
    ...
    def _get_active_agents_for_round(
        ...
        for cfg in agent_configs:
            ...
            # Apply activity-level probability
        ...
        if initial_posts:
            ...
            for post in initial_posts:
                ...
                try:
                    ...
                    print(f"  Warning: failed to create initial post for Agent {agent_id}: {e}")
...
async def main():
    ...
    # Initialize logging (fixed filenames; clear old logs)
...
def setup_signal_handlers():
    ...
    Allows normal resource cleanup (DB, environments, etc.)
    """
