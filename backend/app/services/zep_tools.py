...
Wraps graph search, node reading, edge queries, and related tools for Report Agent.

Core retrieval tools (optimized):
1. InsightForge (deep insight retrieval) - strongest hybrid retrieval; auto-generates sub-questions and multi-dimensional search
2. PanoramaSearch (breadth search) - full overview, including expired content
...
class InsightForgeResult:
    ...
    Contains retrieval results for multiple sub-questions plus integrated analysis.
    """
    query: str
    ...
    def to_text(self) -> str:
        ...
        if self.semantic_facts:
            text_parts.append(f"\n### [Key Facts] (please quote these originals in the report)")
...
class PanoramaResult:
    ...
    def to_text(self) -> str:
        ...
        # Currently valid facts (full output, not truncated)
        ...
        # Related entities (full output, not truncated)
        if self.all_nodes:
            text_parts.append(f"\n### [Involved Entities]")
...
class AgentInterview:
    ...
    def to_text(self) -> str:
        ...
        # Show full agent_bio, not truncated
        ...
        if self.key_quotes:
            ...
            for quote in self.key_quotes:
                ...
                # Filter junk containing numbered question prefixes ('Question N' / Chinese '问题N')
                skip = False
                for d in '123456789':
                ...
                if skip:
                    continue
                # Truncate overly long content (at sentence end, not hard cut)
...
class ZepToolsService:
    ...
    [Core retrieval tools - optimized]
    1. insight_forge - deep insight retrieval (strongest; auto sub-questions; multi-dimensional)
    2. panorama_search - breadth search (full overview, including expired content)
    3. quick_search - simple search (fast retrieval)
    4. interview_agents - deep interview (interview simulated Agents for multi-perspective views)
    ...
    def search_graph(
        ...
        Search related information in the graph using hybrid search (semantic + BM25).
    ...
    def _local_search(
        ...
        Fetch all edges/nodes, then match keywords locally.
    ...
    def get_all_edges(self, graph_id: str, include_temporal: bool = True) -> List[EdgeInfo]:
        """
        Get all edges in the graph (paginated, with temporal info).
    ...
    def get_node_edges(self, graph_id: str, node_uuid: str) -> List[EdgeInfo]:
        ...
        Fetch all graph edges, then filter those related to the specified node.
        ...
        try:
            ...
            result = []
            for edge in all_edges:
                # Check whether the edge relates to the node (as source or target)
    ...
    def get_entity_summary(
        ...
        """
        Get a relationship summary for the specified entity.
        
        Search all information related to the entity and produce a summary.
        ...
        # First search information related to the entity
    ...
    def insight_forge(
        ...
        The strongest hybrid retrieval function; automatically decomposes questions and retrieves across dimensions:
        1. Use LLM to decompose the question into multiple sub-questions
        2. Semantic-search each sub-question
        ...
        # Step 2: Semantic-search each sub-question
        all_facts = []
        all_edges = []
        seen_facts = set()
        ...
        # Get details for all related entities (no count limit; full output)
        entity_insights = []
        node_map = {}  # For later relationship-chain building
    ...
    def _generate_sub_queries(
        ...
        Decompose a complex question into multiple independently retrievable sub-questions.
        """
...
Requirements:
1. Each sub-question should be specific enough that related Agent behaviors or events can be found in the simulated world
...
Return a JSON list of sub-questions."""
    ...
    def panorama_search(
        ...
        Get a full overview, including all related content and historical/expired information:
        1. Fetch all related nodes
        2. Fetch all edges (including expired/invalidated)
        3. Classify and organize currently valid vs historical information
        
        This tool is suited for understanding the full picture of an event and tracing its evolution.
    ...
    def interview_agents(
        ...
        2. Use LLM to analyze the interview requirement and intelligently select the most relevant Agents
        3. Use LLM to generate interview questions
        4. Call /api/simulation/interview/batch for real interviews (dual-platform simultaneous)
        5. Integrate all interview results into an interview report
        
        [Important] Requires the simulation environment to be running (OASIS not shut down)
        
        [Use Cases]
        - Need views on an event from different role perspectives
        - Need to collect multi-party opinions and viewpoints
        - Need real answers from simulated Agents (not LLM-simulated)
        
        Args:
            simulation_id: Simulation ID (to locate persona files and call interview API)
            interview_requirement: Interview requirement description (unstructured, e.g. "Learn students' views on the event")
        ...
        # Add optimization prefix constraining Agent reply format
        INTERVIEW_PROMPT_PREFIX = (
            "You are being interviewed. Based on your persona and all past memories and actions, "
            ...
            "4. Answer questions one by one by number; start each answer with \"Question X:\" (X is the question number)\n"
            "5. Separate answers to different questions with a blank line\n"
            "6. Answers must have substance; answer each question in at least 2-3 sentences\n\n"
        )
        optimized_prompt = f"{INTERVIEW_PROMPT_PREFIX}{combined_prompt}"
        
        # Step 4: Call the real interview API (no platform; default dual-platform interview)
        try:
            ...
            for i, agent_idx in enumerate(selected_indices):
                ...
                # Strategy 2 (supplement): long text inside paired Chinese quotes 「」
                if not key_quotes:
            ...
            result.summary = f"Interview failed: {str(e)}. Simulation environment may be closed; ensure OASIS is running."
            return result
        except Exception as e:
    ...
    def _clean_tool_call_response(response: str) -> str:
        """Clean JSON tool-call wrappers from Agent replies and extract actual content."""
...
3. Choose diverse perspectives (e.g. supporters, opponents, neutrals, professionals)
...
Please select at most {max_agents} Agents most suitable for interview, and explain your reasoning."""
...
Please generate 3-5 interview questions."""
    ...
    def _generate_interview_summary(
        ...
        system_prompt = f"""You are a professional news editor. Based on answers from multiple interviewees, generate an interview summary.
