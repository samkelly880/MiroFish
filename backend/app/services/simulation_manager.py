...
class SimulationState:
    ...
    def get_default_platform(self) -> str:
        """Return default platform based on enable flags"""
        if self.enable_twitter and self.enable_reddit:
            return "reddit"  # Keep original default when both are enabled
...
class SimulationManager:
    ...
    3. Use LLM to generate simulation configuration parameters
    ...
    def prepare_simulation(
        ...
        Steps:
        1. Read and filter entities from the Zep graph
        2. Generate an OASIS Agent Profile for each entity (optional LLM enrichment, supports parallel)
        3. Use LLM to generate simulation config parameters (time, activity, posting frequency, etc.)
        ...
        try:
            ...
            # Reddit was already saved in realtime during generation; save again to ensure completeness
            if progress_callback:
                progress_callback(
            ...
            # Note: run scripts stay in backend/scripts/; they are no longer copied into the simulation directory
