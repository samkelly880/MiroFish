...
Handles retry logic for external API calls such as LLM providers
"""
...
def retry_with_backoff(
    ...
    Args:
        ...
        on_retry: Callback invoked on retry (exception, retry_count)
...
class RetryableAPIClient:
    """
    Wrapper for API clients with retry support
    """
    ...
    def call_batch_with_retry(
        ...
        """
        Process items in batch, retrying each failed item individually
        
        Args:
            items: List of items to process
            process_func: Processing function that takes a single item
            exceptions: Exception types that should trigger a retry
            continue_on_failure: Whether to continue with remaining items after a failure
