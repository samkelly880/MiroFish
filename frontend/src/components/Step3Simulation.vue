...
const props = defineProps({
  simulationId: String,
  maxRounds: Number, // max rounds from Step2
  minutesPerRound: {
    type: Number,
...
const runStatus = ref({})
const allActions = ref([]) // all actions (incremental accumulate)
const actionIds = ref(new Set()) // action ID set for dedupe
const scrollContainer = ref(null)

// Computed
// Show actions in time order (newest at bottom)
...
// Format simulated elapsed time (from round and minutes-per-round)
...
const doStartSimulation = async () => {
  ...
  // Reset all state first so prior simulation cannot interfere
  resetAllState()
...
// Track previous round per platform to detect changes and log
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Detect per-platform round changes and log
      ...
      // Detect simulation completion via runner_status or platform done flags
...
// Check whether all enabled platforms have finished
const checkPlatformsCompleted = (data) => {
  // If no platform data, return false
  if (!data) return false
  
  // Check per-platform completion
  const twitterCompleted = data.twitter_completed === true
  const redditCompleted = data.reddit_completed === true
  
  // If at least one platform finished, check whether all enabled platforms finished
  // Use actions_count to tell if platform is enabled (count > 0 or was running)
  const twitterEnabled = (data.twitter_actions_count > 0) || data.twitter_running || twitterCompleted
  const redditEnabled = (data.reddit_actions_count > 0) || data.reddit_running || redditCompleted
  
  // If no platform enabled, return false
  if (!twitterEnabled && !redditEnabled) return false
  
  // Check that every enabled platform has finished
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Incrementally append new actions (dedupe)
      ...
      // Do not auto-scroll; let user browse the timeline freely
      // New actions append at the bottom
    }
  } catch (err) {
