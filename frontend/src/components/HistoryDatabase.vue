<template>
  ...
  >
    <!-- Background decoration: tech grid lines (only when projects exist) -->
...
let pendingState = null  // pending target state to apply
...
const containerStyle = computed(() => {
  ...
  // Expanded: height from card count
  ...
  // Required height: rows * cardHeight + (rows-1) * gap + small bottom padding
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Count cards in current row to keep each row centered
    ...
    const x = offset * 35
    // Adjust start position near title with suitable spacing
...
// Get style class from round progress
...
// Build title from simulation requirement (first 20 chars)
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Update pending target state (always record latest, even during animation)
        ...
        // If target equals current state, nothing to do
        if (shouldExpand === isExpanded.value) {
          pendingState = null
          return
        }
        
        // Debounce state switches to prevent rapid flicker
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Check whether pending state still needs applying (may have been overridden)
          ...
          setTimeout(() => {
            ...
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Delay briefly before applying to avoid rapid switches
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    },
    {
      ...
      // Adjust rootMargin; shrink viewport bottom so more scroll is needed to expand
...
onMounted(async () => {
  // Load data only after DOM has rendered
  await nextTick()
  await loadHistory()
  
  // Init observer after DOM render
  setTimeout(() => {
    initObserver()
  }, 100)
})

// If using keep-alive, reload data on activate
onActivated(() => {
  loadHistory()
})
...
/* Use CSS background pattern for evenly spaced square grid */
.grid-pattern {
  position: absolute;
  top: 0;
  left: 0;
  ...
  /* Anchor from top-left; height growth only extends bottom without shifting existing grid */
...
.cards-container {
  ...
  /* min-height computed by JS based on card count */
}
