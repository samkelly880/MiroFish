...
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // Transitioned from simulating to not; show end hint
...
const renderGraph = () => {
  ...
  // Stop previous force simulation
  ...
  // Process edge data; compute count and index of edges between the same node pair
  const edgePairCount = {}
  const selfLoopEdges = {} // self-loop edges grouped by node
  const tempEdges = edgesData
    .filter(e => nodeIds.has(e.source_node_uuid) && nodeIds.has(e.target_node_uuid))
  
  // Count edges between each node pair; collect self-loops
  tempEdges.forEach(e => {
  ...
  // Track which edge index we are on for each node pair
  const edgePairIndex = {}
  ...
  tempEdges.forEach(e => {
    ...
    // Check whether edge direction matches normalized direction (source UUID < target UUID)
    const isReversed = e.source_node_uuid > e.target_node_uuid
    
    // Compute curvature: fan out when multiple edges; straight for a single edge
    let curvature = 0
    if (totalCount > 1) {
      ...
      // If edge direction is opposite of normalized, flip curvature
      // Ensures all edges share one reference frame and do not overlap due to direction
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  // Simulation - dynamically adjust node distance by edge count
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // Dynamically adjust distance by edge count between this node pair
      // Base distance 150, plus 40 per extra edge
    ...
    // Add center gravity so independent node clusters gather toward the center
  ...
  const getLinkPath = (d) => {
    ...
    if (d.isSelfLoop) {
      ...
      const loopRadius = 30
      // Start from the right of the node and loop back
      const x1 = sx + 8  // start offset
      const y1 = sy - 4
      const x2 = sx + 8  // end offset
      const y2 = sy + 4
      // Draw self-loop with arc (sweep-flag=1 clockwise)
    ...
    // Offset perpendicular to the link, scaled by distance so the curve stays visible
    // More edges => larger offset ratio relative to distance
    const pairTotal = d.pairTotal || 1
    const offsetRatio = 0.25 + pairTotal * 0.05 // Base 25%, plus 5% per extra edge
  ...
  // Compute curve midpoint (for label placement)
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        // Detect real drag start (movement exceeds threshold)
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // Restart simulation only when a real drag is first detected
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // Only cool down simulation after a real drag
        if (d._isDragging) {
