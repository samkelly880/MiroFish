            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (content done, but section may not be fully complete) -->
                  ...
                  <!-- Section Complete (section generation finished) -->
...
const toggleRawResult = (timestamp, event) => {
  ...
  // After DOM update, adjust scroll to keep button in the same place
...
const toolConfig = {
  'insight_forge': {
    name: 'Deep Insight',
    color: 'purple',
    icon: 'lightbulb' // lightbulb icon - insights
  },
  'panorama_search': {
    name: 'Panorama Search',
    color: 'blue',
    icon: 'globe' // globe icon - panoramic search
  },
  'interview_agents': {
    name: 'Agent Interview',
    color: 'green',
    icon: 'users' // users icon - conversation
  },
  'quick_search': {
    name: 'Quick Search',
    color: 'orange',
    icon: 'zap' // zap icon - quick
  },
  'get_graph_statistics': {
    name: 'Graph Stats',
    color: 'cyan',
    icon: 'chart' // chart icon - stats
  },
...
// Parse functions
// Platform no-reply placeholders (legacy Chinese + current English backend)
...
const parseInsightForge = (text) => {
  ...
  try {
    // Extract analysis question (CN / EN)
    ...
    const subQSection = text.match(/### (?:分析的子问题|Analyzed Sub-questions)\n([\s\S]*?)(?=\n###|$)/)
    if (subQSection) {
    ...
    // Extract core entities - full extract with summary and related fact count
...
const parseInterview = (text) => {
  ...
  try {
    // Extract interview topic (CN / EN)
    const topicMatch = text.match(/\*\*(?:采访主题|Interview topic):\*\*\s*(.+?)(?:\n|$)/)
    if (topicMatch) result.topic = topicMatch[1].trim()
    
    // Extract interviewee count (e.g. "5 / 9 位模拟Agent" or "5 / 9 simulated Agents")
    const countMatch = text.match(/\*\*(?:采访人数|Interviewees):\*\*\s*(\d+)\s*\/\s*(\d+)/)
    if (countMatch) {
    ...
    const reasonMatch = text.match(/### (?:采访对象选择理由|Interviewee Selection Reasoning)\n([\s\S]*?)(?=\n---\n|\n### (?:采访实录|Interview Transcript))/)
    if (reasonMatch) {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // Format 2: - 选择名字（index X）：理由 / - Selected name (index X): reason
        // Example: - 选择家长_601（index 0）：作为家长群体代表...
        if (!headerMatch) {
          headerMatch = line.match(/^-\s*(?:选择|Selected?\s+)([^（(]+)(?:[（(]index\s*=?\s*\d+[)）])?[：:]\s*(.*)/i)
          if (headerMatch) {
            name = headerMatch[1].trim()
            reasonStart = headerMatch[2]
          }
        ...
        } else if (currentName && line.trim() && !line.match(/^未选|^综上|^最终选择|^Not selected|^In summary|^Finally selected/i)) {
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        // Separate Twitter and Reddit answers (CN 【】 / EN [])
        const twitterMatch = answerText.match(/(?:【Twitter平台回答】|\[Twitter platform answer\])\n?([\s\S]*?)(?=【Reddit平台回答】|\[Reddit platform answer\]|$)/)
        const redditMatch = answerText.match(/(?:【Reddit平台回答】|\[Reddit platform answer\])\n?([\s\S]*?)$/)
        ...
        // Platform fallback (compat with old format: only one platform marker)
        if (!twitterMatch && redditMatch) {
          // Reddit-only answer; copy as default only when not placeholder text
          ...
          // No per-platform marker (very old format); use whole text as answer
          interview.twitterAnswer = answerText
        }
      }
      
      // Extract key quotes (compat with multiple quote styles)
      const quotesMatch = block.match(/\*\*(?:关键引言|Key quotes):\*\*\n([\s\S]*?)(?=\n---|\n####|$)/)
      if (quotesMatch) {
        const quotesText = quotesMatch[1]
        // Prefer > "text" format
        let quoteMatches = quotesText.match(/> "([^"]+)"/g)
        // Fallback: match > "text" or curly/Chinese quotes
        if (!quoteMatches) {
    ...
    const summaryMatch = text.match(/### (?:采访摘要与核心观点|Interview Summary and Key Viewpoints)\n([\s\S]*?)$/)
    if (summaryMatch) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    // Extract search query (CN / EN)
    const queryMatch = text.match(/(?:搜索查询|Search query):\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()
    
    // Extract result count
    const countMatch = text.match(/(?:找到\s*(\d+)\s*条|Found\s+(\d+)\s+related items)/)
    ...
    // Try extracting edge info (if any)
    ...
    // Try extracting node info (if any)
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Check whether text is a platform placeholder (CN + EN)
    const isPlaceholderText = isPlatformPlaceholder

    // Try splitting answers by question number
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // Support numbering formats:
      // 1. "问题X：" / "Question X:" (CN / EN backend)
      // 2. "1. " or "\n1. " (number+dot, legacy compat)
      let matches = []
      let match

      // Prefer "问题X：" / "Question X:" format first
      const labeledPattern = /(?:^|[\r\n]+)(?:问题|Question)\s*(\d+)[：:]\s*/gi
      while ((match = labeledPattern.exec(answerText)) !== null) {
        matches.push({
      ...
      // If no/only one number found, return whole text
      if (matches.length <= 1) {
    ...
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // Split failed: first question gets full answer; others empty
      return qIdx === 0 ? answer : ''
    }
    
    // Check whether a question has dual-platform answers (filter placeholders)
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // Both platforms have real answers (non-placeholder) and differ
    ...
    return () => h('div', { class: 'interview-display' }, [
      ...
      props.result.interviews.length > 0 && h('div', { class: 'interview-detail' }, [
        ...
        // Q&A Conversation Thread - Q then A style
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual-platform toggle (only when real dual answers exist)
...
const activeStep = computed(() => {
  ...
  // If none active, return last done step
...
const renderMarkdown = (content) => {
  ...
  // Strip leading H2 (## xxx) since section title is shown outside
  ...
  // Strip whitespace after list open tags
  html = html.replace(/<ul class="md-ul">\s+/g, '<ul class="md-ul">')
  html = html.replace(/<ol class="md-ol">\s+/g, '<ol class="md-ol">')
  // Strip whitespace before list close tags
  ...
  // Process newlines - blank lines become paragraphs, single newlines become <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  // Strip <br> around block elements
  html = html.replace(/<br>\s*(<ul|<ol|<blockquote)/g, '$1')
  html = html.replace(/(<\/ul>|<\/ol>|<\/blockquote>)\s*<br>/g, '$1')
  // Strip <p><br> immediately before block elements (extra blank lines)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Collapse consecutive <br> tags
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Strip <br> before a paragraph that follows a block element
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix non-contiguous ordered-list numbering: keep incrementing when single-item <ol>s are separated by paragraphs
...
const getLogLevelClass = (log) => {
  if (/ERROR|错误/i.test(log)) return 'error'
  if (/WARNING|警告/i.test(log)) return 'warning'
  // INFO uses default color; do not mark as success
  return ''
}
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'section_complete') {
            if (log.details?.content) {
              ...
              // Auto-expand the section just generated
          ...
          if (log.action === 'report_complete') {
            ...
            stopPolling()
            // Unified scroll logic in nextTick after the loop
          }
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // If task done, scroll to top; else scroll to bottom to follow latest logs
            if (isComplete.value) {
...
const extractFinalContent = (response) => {
  ...
  // Try extracting content inside <final_answer>
  ...
  // Try finding content after Final Answer: / 最终答案: (EN / CN)
  const finalAnswerMatch = response.match(/(?:Final\s*Answer|最终答案)[:：]\s*\n*([\s\S]*)$/i)
  if (finalAnswerMatch) {
    return finalAnswerMatch[1].trim()
  }
  
  // If starts with ## / # / >, may be direct markdown
  const trimmedResponse = response.trim()
  if (trimmedResponse.match(/^[#>]/)) {
    return trimmedResponse
  }
  
  // If content is long and looks like markdown, strip thinking then return
  if (response.length > 300 && (response.includes('**') || response.includes('>'))) {
    // Remove Thought: thinking preamble
