    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <!-- Simulation rounds config - shown only after config is generated and rounds calculated -->
...
const props = defineProps({
  simulationId: String,  // passed from parent
  projectData: Object,
  graphData: Object,
  systemLogs: Array
})
...
// State
const phase = ref(0) // 0: init, 1: generate profiles, 2: generate config, 3: done
...
// Log dedupe: remember last related message
let lastLoggedMessage = ''
let lastLoggedProfileCount = 0
let lastLoggedConfigStage = ''

// Simulation rounds config
const useCustomRounds = ref(false) // use auto-configured rounds by default
...
// Compute auto rounds from config (no hardcoded default)
...
// Count total related topics across all profiles
...
const handleStartSimulation = () => {
  // Build params to pass to parent
  const params = {}
  
  if (useCustomRounds.value) {
    // User custom rounds; pass max_rounds
    params.maxRounds = customMaxRounds.value
    addLog(t('log.startSimCustomRounds', { rounds: customMaxRounds.value }))
  } else {
    // User keeps auto rounds; do not pass max_rounds
...
const startPrepareSimulation = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Immediately set expected agent total (from prepare API response)
...
const fetchProfilesRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Update only when API returns a valid value; avoid overwriting existing valid values
      ...
      // Emit Profile generation progress log (only when count changes)
...
const fetchConfigRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Emit config generation stage logs (avoid duplicates)
