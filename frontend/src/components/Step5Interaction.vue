...
const chatHistoryCache = ref({}) // cache all chat histories: { 'report_agent': [], 'agent_0': [], ... }
...
const sendToAgent = async (message) => {
  ...
  if (res.success && res.data) {
    ...
    // Convert object dict to array; prefer reddit replies
    let responseContent = null
    const agentId = selectedAgentIndex.value
    
    if (typeof resultsDict === 'object' && !Array.isArray(resultsDict)) {
      // Prefer reddit reply, then twitter
...
/* Fix ordered-list numbering - CSS counters so multiple ols continue */
.message-text {
...
/* Blockquote styles in chat/survey area */
