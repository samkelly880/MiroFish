<template>
  <div class="process-page">
    ...
    <div class="main-content">
      <!-- Left: realtime graph display -->
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        <div class="panel-header">
          <div class="header-left">
            <span class="header-deco">◆</span>
            <span class="header-title">Realtime Knowledge Graph</span>
          </div>
        ...
        <div class="graph-container" ref="graphContainer">
          <!-- Graph visualization (show whenever data exists) -->
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-hint">Graph build will start automatically when generation finishes</p>
          </div>
            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  After uploading documents, the LLM analyzes content and auto-generates an ontology suited for opinion simulation (entity types + relation types)
                </div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">API description</div>
                <div class="detail-content">
                  Based on the generated ontology, chunk documents and call Zep API to build a knowledge graph and extract entities/relations
                </div>
              </div>
...
// Current project ID (may change from 'new' to real id)
...
const handleNewProject = async () => {
  ...
  if (!pending.isPending || pending.files.length === 0) {
    error.value = 'No pending files to upload; please return to the homepage and try again'
    loading.value = false
    return
  }
...
const startGraphPolling = () => {
  ...
  // Auto-fetch graph data every 10s
...
const fetchGraphData = async () => {
  try {
    // First fetch project info for graph_id
    const projectResponse = await getProject(currentProjectId.value)
...
const renderGraph = () => {
  ...
  simulation.on('tick', () => {
    // Update all edge positions (visible lines and transparent hit areas)
