...
// Read maxRounds from query at init so child gets the value immediately
...
const handleGoBack = async () => {
  // Before returning to Step 2, stop any running simulation
  ...
  try {
    // First try graceful close of simulation env
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      // Env not running; check whether process still needs stopping
...
const loadSimulationData = async () => {
  try {
    ...
    if (simRes.success && simRes.data) {
      ...
      // Fetch simulation config for minutes_per_round
      try {
...
const loadGraph = async (graphId) => {
  // While simulating, auto-refresh without fullscreen loading to avoid flicker
...
const startGraphRefresh = () => {
  ...
  // Refresh once immediately, then every 30s
...
onMounted(() => {
  ...
  // Record maxRounds config (value already read from query at init)
  if (maxRounds.value) {
